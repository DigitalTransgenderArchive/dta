  ...
  def self.find(subject)
    ...
    if matches.present?
      ...
      return matches.map! { |item|
        ##{URI.escape(item)}
        full_uri = 'https://homosaurus.org/v3/' + item['identifier_ssi']
  ...
  def self.getBroader(broader_uris)
    ...
    if broader_uris.present?
      broader_uris.each do |broader_single_uri|
        broader_label = DSolr.find({q: "id:#{solr_clean("homosaurus/v3/#{broader_single_uri}")} AND model_ssi:HomosaurusV3", rows: '1', fl: 'prefLabel_ssim' })
        broader_list << {:uri_link=>"https://homosaurus.org/v3/#{broader_single_uri.split('/').last}", :label=>broader_label[0]["prefLabel_ssim"][0]}
      end
    end
  ...
  def self.getNarrower(narrower_uris)
    ...
    if narrower_uris.present?
      narrower_uris.each do |narrower_single_uri|
        narrower_label = DSolr.find({q: "id:#{solr_clean("homosaurus/v3/#{narrower_single_uri}")} AND model_ssi:HomosaurusV3", rows: '1', fl: 'prefLabel_ssim' })
        narrower_list << {:uri_link=>"https://homosaurus.org/v3/#{narrower_single_uri.split('/').last}", :label=>narrower_label[0]["prefLabel_ssim"][0]}
      end
    end
